...
impl SentenceTransformer {
    ...
    pub fn encode(&self,
                  sentences: Vec<&str>,
                  batch_size: Option<usize>) -> Vec<Vec<f64>>
    {
        let batch_size = if let Some(value) = batch_size {value} else {8};
        ...
        for batch_idx in (0..sentences.len()).step_by(batch_size) {
            ...
            for text in &batch_tokens {
                let (input_ids, token_type_ids, input_mask, _sentence_length) = self.bert.get_sentence_features(&text, longest_seq);
            ...
            all_embeddings.extend(Vec::<Vec<f64>>::from(&features.sentence_embedding.unwrap()));
        }
